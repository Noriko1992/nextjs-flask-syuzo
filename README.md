...
〇GitHubのクローン
```bash
git clone https://github.com/Noriko1992/syuzo0926.git
```
...
2.6 Tailwind CSSをインストールする(以下は初回のみ！！！)
```bash
npm install -D tailwindcss postcss autoprefixer
npx tailwindcss init -p
```

2.7 Tailwindの設定を追加する（Gitからクローンを作っていたらこの過程は飛ばしてOK）
tailwind.config.jsを開き、以下のようにcontentを設定します。
```tailwind.config.jsフォルダ
...
module.exports = {
  ...
  theme: {
    extend: {
      fontFamily: {
        'Yuji_Syuku': ["var(--font-Yuji_Syuku)"],
      },
    },
  },
  plugins: [],
...
2.8 CSSファイルを作成する（Gitからクローンを作っていたらこの過程は飛ばしてOK）
→styles フォルダの作成
　frontend フォルダの直下に styles というフォルダを作成。
styles フォルダの中に globals.css というファイルを作成し、以下の内容を追加。
```globals.cssフォルダ
@tailwind base;
@tailwind components;
@tailwind utilities;
...
2.9 pages/_app.jsにグローバルCSSをインポートする(Gitからクローンを作っていたらこの過程は飛ばしてOK)
→Next.jsの特別なファイルとして、pages フォルダ内に _app.js というファイルを作成。このファイルはアプリ全体に適用される設定を行う場所です。
pages/_app.jsを開き、以下のようにglobals.cssをインポート。
```pages/_app.jsフォルダ
...
2.9.1 コンポーネントをTailwind CSSでスタイリングする(Gitからクローンを作っていたらこの過程は飛ばしてOK)
もしまだ tailwind.config.js が作成されていない場合は、以下のコマンドを実行して生成します。
```bash
npx tailwindcss init -p
```
